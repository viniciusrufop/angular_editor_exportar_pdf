import { Component, OnInit, ElementRef, ViewChild, ChangeDetectorRef, AfterViewInit } from '@angular/core';
import { FormBuilder, FormGroup } from '@angular/forms';
import { DrawOptions, drawDOM, Group, exportPDF } from '@progress/kendo-drawing';
import { saveAs } from '@progress/kendo-file-saver';

declare const ClassicEditor: any;

@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.scss']
})
export class AppComponent implements OnInit, AfterViewInit {

  @ViewChild('post', {static: false}) public post: ElementRef;
  @ViewChild('ckEditor', {static: false}) public ckEditor: ElementRef;
  public Editor = ClassicEditor;

  public textoInicial = `
  <figure class="image image_resized" style="width:82px;"><img src="data:image/png;base64," alt="Angular (framework) – Wikipédia, a enciclopédia livre"></figure><h4><span style="color:hsl(270,75%,60%);">Editor Utilizado</span></h4><p><span style="color:hsl(0,0%,0%);font-size:12px;">CKEditor → </span><a href="https://ckeditor.com/ckeditor-5/">https://ckeditor.com/ckeditor-5/</a></p><h4><span style="color:hsl(270,75%,60%);">Bibliotecas para fazer download do PDF</span></h4><p><a href="https://www.telerik.com/kendo-angular-ui/components/drawing/"><span style="color:hsl(0,0%,0%);font-size:12px;">Drawing →</span><span style="color:hsl(0,0%,0%);"> </span>https://www.telerik.com/kendo-angular-ui/components/drawing/</a></p><p><a href="https://www.telerik.com/kendo-angular-ui/components/filesaver/"><span style="color:hsl(0,0%,0%);font-size:12px;">File Saver →</span> https://www.telerik.com/kendo-angular-ui/components/filesaver/</a></p><h4><span style="color:hsl(270,75%,60%);">Como Instalar</span></h4><p><span style="color:hsl(0,0%,0%);font-size:12px;">Copiar a pasta <strong>ckeditor </strong>para dentro de <strong>assets</strong>, incluir no arquivo <strong>angular.json</strong> o código</span></p><pre><code>"scripts": [
	"./src/assets/ckeditor/build/ckeditor.js"
]</code></pre><p><span style="font-size:12px;">Instalar as dependências</span></p><pre><code>npm install @ckeditor/ckeditor5-angular @progress/kendo-drawing @progress/kendo-file-saver</code></pre><p><span style="font-size:12px;">Adicionar nos <strong>imports</strong> de <strong>app.module.ts</strong></span></p><pre><code>import { CKEditorModule } from '@ckeditor/ckeditor5-angular';

imports: [
	...
    CKEditorModule,
    ...
  ]</code></pre>
  `;

  public form: FormGroup;
  public showPost: boolean = false;
  public optionsPDF : DrawOptions = {
    forcePageBreak : '.page-break',
    paperSize: "A4",
    margin: { right: '0.3cm', left: '0.3cm' }

  }

  constructor(
    private formBuilder: FormBuilder,
    private changeDetectorRef: ChangeDetectorRef
  ) {}

  ngOnInit() {
    this.createForm();
  }

  ngAfterViewInit() {
    this.salvar();
  }

  createForm(): void {
    this.form = this.formBuilder.group({
      post: this.textoInicial
    });
  }

  salvar(el?: HTMLElement): void {
    this.showPost = (this.form.get('post').value !== '');

    this.changeDetectorRef.detectChanges();

    this.post.nativeElement.innerHTML = this.form.get('post').value;

    console.log(this.form.get('post').value);

    if (el) el.scrollIntoView({block: 'start', behavior: 'smooth'});
  }

  limpar(): void {
    this.form.get('post').setValue('');
  }

  downloadPDF() {
    drawDOM(this.post.nativeElement, this.optionsPDF).then((group: Group) => {
      return exportPDF(group);
    }).then((dataUri) => {
        let pdfName = `Exemplo.pdf`;
        saveAs(dataUri, pdfName);
    });
  }
}
